...
# Shadow price of power balance constraint = marginal price at slack bus
lambda_slack = constraints[0].dual_value

print(f"\n--- Slack Bus LMP (Bus 1) = {lambda_slack:.4f} $/MWh")

# Approximate LMPs at other buses
# λ_i = λ_slack + Σ_k PTDF[k, i] × (dual upper_k - dual lower_k)
...
for i in range(num_buses):
    print(f"Bus {i+1}: LMP = {lmp[i]:.4f} $/MWh")
