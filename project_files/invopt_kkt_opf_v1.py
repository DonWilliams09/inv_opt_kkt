...
complementary = [
    cp.multiply(mu_min, Pg_min - Pg) == 0,
    cp.multiply(mu_max, Pg - Pg_max) == 0,
    cp.multiply(nu_max, PTDF @ P_inj_reduced - branch_data_congested[:, 5]) == 0,
    cp.multiply(nu_min, - branch_data_congested[:, 5] - PTDF @ P_inj_reduced) == 0
]
...
inv_prob.solve(solver=cp.MOSEK)
